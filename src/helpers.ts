...
import { SwipeCardProps, Speed, Coordinate, TemporalCoordinate } from './types';
...
export const calcDirection = (speed: Speed): SwipeDirection => {
    if (Math.abs(speed.x) > Math.abs(speed.y)) {
        if (speed.x >= 0) return SwipeDirection.RIGHT;
        else return SwipeDirection.LEFT;
    } else {
        if (speed.y >= 0) return SwipeDirection.UP;
...
export const mouseCoordinates = (event: MouseEvent): Coordinate => ({
    x: event.clientX,
    y: event.clientY
});
export const touchCoordinates = (event: TouchEvent): Coordinate => ({
